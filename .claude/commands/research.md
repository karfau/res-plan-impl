...
```
I'm ready to research some code. Please provide your research question or area of interest, and I'll analyze it thoroughly by exploring relevant components and connections.
```
...
2. **Analyze and decompose the research question:**
   ...
   - Create a research plan using the task list tool to track all subtasks
...
5. **Gather metadata for the research document:**
   - Run the following to collect metadata:
     ...
     - `git remote get-url origin` — remote URL (derive repository name as `owner/repo` from this)
   - Resolve `current-task` to the real TASK_FOLDER path (e.g. `readlink -f current-task`) and save the file there
   - Filename format: `research-YYYY-MM-DD-description.md` where:
     - YYYY-MM-DD is today's date
     - description is a brief kebab-case description of the research topic
   - Example: `research-2025-01-08-authentication-flow.md`
...
6. **Generate research document:**
   ...
   - Structure the document with YAML frontmatter followed by content:
     ...
     researcher: [Researcher name from git author config]
     ...
     last_updated_by: [Researcher name from git author config]
     ---
     ...
     ## Related Research
     [Links to related research documents in this task folder]
...
7. **Add GitHub permalinks (if applicable):**
   - For each file reference, determine which repository it belongs to:
     - **Files in the current repo**: use metadata from step 5 (remote URL already collected)
       - Derive the GitHub owner/repo from the remote URL
       - Permalink: `https://github.com/{owner}/{repo}/blob/{commit}/{file}#L{line}`
     - **Files inside `code/` symlinks**: each symlink may point to a separate git repo
       - Resolve the symlink: `readlink -f code/{name}`
       - From the resolved path, get that repo's metadata:
         - `git -C {resolved_path} rev-parse HEAD` — commit hash
         - `git -C {resolved_path} remote get-url origin` — remote URL
       - Derive the GitHub owner/repo from the remote URL
       - Permalink: `https://github.com/{owner}/{repo}/blob/{commit}/{relative_file_path}#L{line}`
         where `relative_file_path` is the path relative to that repo's root
   - For any repo where the referenced commit has not been pushed, print a single warning reminding the user to push.
   - Replace local file references with permalinks where available
...
9. **Handle follow-up questions:**
   ...
   - Update frontmatter: set `last_updated` to today's date, `last_updated_by` to git author name, and set (or update) `last_updated_note: "Added follow-up research for [brief description]"`
